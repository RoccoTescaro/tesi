...
#The following code has been modified from the original version.
#The original code can be found at: https://github.com/kynkaat/improved-precision-and-recall-metric
#The changes consist of adapting the code to tensorflow 2.17 and removed print statements.
#Rocco Tescaro - 4 September 2024
...
class DistanceBlock:
    ...
    def __init__(self, num_features, num_gpus):
        ...
        # Define the function that will compute distances
        self._distance_function = tf.function(self._compute_distances)

    def _compute_distances(self, features_batch1, features_batch2):
        """Compute pairwise distances between two batches of feature vectors."""
        features_split2 = tf.split(features_batch2, self.num_gpus, axis=0)
        distances_split = []

        for gpu_idx in range(self.num_gpus):
            with tf.device('/GPU:%d' % gpu_idx):
                distances_split.append(batch_pairwise_distances(features_batch1, features_split2[gpu_idx]))

        return tf.concat(distances_split, axis=1)
    ...
    def pairwise_distances(self, U, V):
        ...
        return self._distance_function(U, V)
